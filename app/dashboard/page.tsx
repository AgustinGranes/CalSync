...
export default function Dashboard() {
  ...
  // Sync global loading: only stop when both auth is ready AND config is loaded (or error)
  useEffect(() => {
    if (!loading && (!!config || !!configError)) {
      setGlobalLoading(false);
    }
  }, [loading, config, configError, setGlobalLoading]);
  ...
  if (loading || (!config && !configError)) {
    // Return nothing while loading as the GlobalLoader in layout handles the UI.
    return null;
  }
